...
/*
 * Return a safe provider/application error message.
 *
 * We intentionally allow a provider message through when it
 * came from VTU.ng because messages such as:
 *
 *   INVALID FIELD. Please, enter a valid service_id.
 *
 * are useful for diagnosing the integration.
 *
 * We never expose:
 * - access tokens
 * - Firebase tokens
...
function getSafeErrorMessage(error) {
  ...
  /*
   * Provider rejection messages are useful to the customer
   * and to us when testing the integration.
   *
   * VtuProviderError already classifies these as
   * provider_rejection.
   */
  if (
  ...
  /*
   * These messages are generated by our VTU adapter and are
   * safe to show because they contain no credentials or raw
   * provider payload.
   */
...
/*
 * Send a transaction response in one consistent format.
 */
