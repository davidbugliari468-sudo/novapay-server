...
/*
 * Only used for an HTTP response from the purchase endpoint.
 *
 * A definite provider-side rejection must become "failed" so the existing
 * data service releases the NovaPay wallet reservation.
 *
 * Transport ambiguity, authentication failure, rate limiting and server
 * errors are deliberately NOT handled here. Those continue to throw and
 * therefore remain protected by the existing unknown/reconciliation flow.
 */
function normalizePurchaseHttpFailure(error) {
  ...
  /*
   * 400-level purchase rejection means BabsPay rejected the purchase
   * request itself. It is safe to release the NovaPay reservation.
   *
   * 409/422 are also treated as definite request/business rejection unless
   * the response is one of the explicit ambiguous cases above.
   */
  if (
    definiteFailure ||
    status === 400 ||
    ...
    return {
      ...
      providerPhone: null,
      providerAmountNaira: null,
      identityVerified: false,
      identityMismatches: [],
      response: providerResponse,
      errorCode: providerCode || "BABSPAY_TRANSACTION_FAILED",
      errorMessage:
        providerMessage || "BabsPay rejected the data purchase.",
    };
  }
...
async function purchaseData({
  ...
  try {
    ...
    /*
     * This is the only change to the purchase flow.
     *
     * A definite BabsPay business/request rejection becomes "failed",
     * allowing the existing service layer to release the reservation.
     *
     * Timeouts, network errors, authentication errors, rate limits and
     * server errors still throw and therefore remain UNKNOWN.
     */
    const normalizedFailure = normalizePurchaseHttpFailure(error);
